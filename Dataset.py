...
class Dataset():
    def __init__(self, folders):
        ...
        if bool(folders.get('-IMG-')):
            ...
            self.img_samples = natsorted(os.listdir(self.img_dir))

        if bool(folders.get('-BIN-')):
            self.binary_dir = folders['-BIN-']
            self.binary_samples = natsorted(os.listdir(self.binary_dir))

        if bool(folders.get('-DATA-')):
            self.data_dir = folders['-DATA-']
            self.data_samples = natsorted(os.listdir(self.data_dir))
